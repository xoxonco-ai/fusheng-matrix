...
2026-08-27，ingest 操作 `ingest-paipan-fix-20260827` 完成（防護字串已修補並重新擷取）。
...
- **唯一落差已修補**：父母線索防護字串補成「嚴禁猜姓氏或具體事件」，文件與實作現在一致。稽核前後兩份快照都保留在 `.raw/captured/`。
...
- **可修**：三處重複實作收斂成單一函式，避免漂移——這是目前最大的實作風險。
- **待抽查**：防護字串存在不等於模型遵守，要拿實際產出的報告抽驗父母段落。
